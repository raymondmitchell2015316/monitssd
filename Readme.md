...
- `--admin`            Start admin web interface (default port: 8080).
- `--admin-port`       Set custom port for admin web interface.
...
The application now includes a modern web-based admin panel for easy configuration and monitoring!

### Start Admin UI

```bash
# Start on default port 8080
./evilginx_monitor --admin

# Start on custom port
./evilginx_monitor --admin --admin-port 9000
```
...
For deploying to a VPS, see the comprehensive guides:

- **[QUICK_START.md](QUICK_START.md)** - Fast deployment guide
- **[DEPLOYMENT.md](DEPLOYMENT.md)** - Detailed deployment instructions
- **[DOMAIN_SETUP.md](DOMAIN_SETUP.md)** - Set up custom domain with SSL
...
# 3. On VPS: Set up as systemd service
# See DEPLOYMENT.md for full instructions
```

## 🔧 Configuration

To set up notifications, you can:
1. **Use the Admin Web UI** (recommended) - Access at `http://localhost:8080`
2. **Use Interactive Mode** - Run the app and use commands

Example for enabling Telegram via interactive mode:
```bash
